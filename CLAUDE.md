...
never branches on a kind literal. Walls are the second family on the same
catalog machinery (`walls.ts`, `wallConfigFor`); new families (power-ups,
turrets, bosses) should ride `createCatalog` rather than invent parallel
plumbing. The **authoritative balance scalars live in `config/data/balance.json`**;
`constants.ts` derives from it and stays the sim's one import site, so retuning
is a two-file edit — the JSON entry and its literal pin in `constants.test.ts`.
`decideAi` routes by the resolved profile's `behavior`; grey's dodge patience is
`(1 − aggression) · TICK_HZ`, pinned equal to `DODGE_PATIENCE_TICKS` in
...
`minimumDistance`, `retreatChance`, and every chance magnitude. The 9-type Wii
taxonomy in `config/reference/` is reference data only — nothing in the game
reads it.
