...
- Dev-flag retirement: `FLAG_REGISTRY` names 48 flags. Each one that exists to settle an open question needs an owner's ship-or-delete decision, which is the arrangement's own stated rule; a flag kept as a permanent rollback lever is exempt and says so in its own header (`.claude/rules/game.md`). #50 *(prose-only PR)*
