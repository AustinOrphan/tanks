...
**Raised 2026-08-03**, reviewing PR #80. Everything here is measured against the live
`austinorphan.com` Pages infrastructure using sibling project sites of the same account,
because `/tanks/` was not yet deployed.

**1. The ten missing audio files cost 91.6 kB on every single load, and it never caches.**
`public/audio/` has only `.gitkeep`; no `.wav` has ever been committed
...
emits `loaderror` immediately without a retry, which is the cheap failure shape. The cost
is the traffic. GitHub Pages sets no `cache-control` on a `.wav` 404 (0 of 3 probed) while
`.mp3`, `.ogg`, `.js` and `.png` 404s all get `max-age=14400` (4 of 4), and a Pages 404
body is 9,379 bytes. So: 9,379 × 10 = **91.6 kB and 10 round trips per load, never
cached** — about half the 186 kB gzipped bundle. Fix is a decision, not a patch: either
render and commit the ten wavs, or stop the manifest requesting files that are not there.
Note `npm run audio` does **not** do the first — it writes to the gitignored `audio-out/`,
needs chromium, and names files `${label}.wav`, not `public/audio/cannon.wav`.
...
Harmless today — its precache list is `/`, `/blog`, `/rss.xml`, and `caches.match` is
keyed by exact URL, so every game request falls through to `fetch`. But it means any
offline/precache work in this repo would be silently wiped whenever a player visits the
portfolio, and if that worker ever gains runtime caching the game would serve stale hashed
assets with no way for this pipeline to invalidate them. Not fixable from here.
**UNVERIFIED:** the scope is derived from the spec and the absence of the header, not
measured in a browser — `navigator.serviceWorker.getRegistrations()` on a deployed
`/tanks/` page would settle it.

**3. `http://` and `https://` are different localStorage origins.** Project pages under the
custom domain ship with `https_enforced: false` and plain HTTP is served without a redirect
(`http://austinorphan.com/learnimals/` → 200, 0 hops), while the apex itself does redirect.
All four save keys (`tanks.progress.v1`, `tanks.stats.v1`, `tanks.achievements.v1`,
`tanks.custom.v1`) are origin-scoped, so anything a player builds on the http origin
vanishes the day HTTPS is enforced. Enforcing it *before* the first deploy avoids this
entirely, which is why it was done that way.
