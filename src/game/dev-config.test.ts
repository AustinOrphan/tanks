...
describe('explainDevConfig', () => {
  ...
  it('THE PREMISE the rejection rule rests on: every valued flag defaults to null', () => {
    ...
    expect(valued.length, 'the population this premise covers').toBe(29); // +1: hudFont (#865's typeface arms), measured 28 -> 29; +1: enemyRole (#773's role cue), measured 27 -> 28; +1: stockCue (#230's cue arms), measured 26 -> 27; +4: #735's render overrides; +1 each: gallery, aiPerception, blockedFire, menuTransition, topbar, outcome, identityMarker, arrival, versusActions, shellTrail, bench
