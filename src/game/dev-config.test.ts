...
describe('control type is derived from the registry, not declared twice', () => {
  ...
  it('assigns a control kind to every field, and every boolean field is a toggle', () => {
    ...
    expect(controls).toHaveLength(34); // +1 each: aiPerception (#359's superseded bound), aiContact (#372's contact overlay), blockedFire (#356's cue arms), menuTransition (#542's menu transitions), topbar (#552's topbar arms), pp1Roles (#358's role-matrix arm), outcome (#591's capture arm), identityMarker (#630's two identity-channel candidates)
  });
...
describe('explainDevConfig', () => {
  ...
  it('THE PREMISE the rejection rule rests on: every valued flag defaults to null', () => {
    ...
    expect(valued.length, 'the population this premise covers').toBe(17); // +1 each: aiPerception, blockedFire, menuTransition, topbar, outcome, identityMarker
