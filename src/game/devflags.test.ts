...
describe('parseDevFlags: blockedFire values are safe to paste into a query string (issue #497)', () => {
  it('every documented cue pastes raw into ?dev=1&blockedFire=... and comes back as itself -- population: the 16 registered values', () => {
    // 5 when this was written for issue #497, and 16 now. Issue #516 built the comparison
    // matrix and the owner's "maybe a smoke type texture?" added `smoke` on top of it;
    // then the vocabulary shrank twice, as the owner adopted an arm and it stopped being
    // selectable -- #526 retired `turret` by making the gun recoil unconditional, and #536
    // retired `smoke` the same way. The number is asserted so a cue added without a
    // query-safe spelling fails here rather than silently joining the sweep below.
