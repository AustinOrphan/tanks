...
describe('startGameWith: dev flags stay off by default', () => {
  ...
  it('asks the renderer for every dev overlay OFF by default', () => {
    ...
    const off = boot();
    expect(off.rec.rendererArgs[0][4]).toEqual({ aimRay: false, mineReach: false, mineTimer: false, aiContact: false, blockedFire: null, playerColor: '#hex-blue', playerSkin: 'solid', playerAccent: null, quality: QUALITY_PRESETS.high, enemyDeathPulse: false, mineWarn: null, identityMarker: null, shellTrail: null, arrival: null, enemyRole: null, wreck: null });
    off.handle.dispose();
  });
  ...
  it('asks for each overlay only when its own flag is on', () => {
    ...
    expect(ray.rec.rendererArgs[0][4]).toEqual({ aimRay: true, mineReach: false, mineTimer: false, aiContact: false, blockedFire: null, playerColor: '#hex-blue', playerSkin: 'solid', playerAccent: null, quality: QUALITY_PRESETS.high, enemyDeathPulse: false, mineWarn: null, identityMarker: null, shellTrail: null, arrival: null, enemyRole: null, wreck: null });
    ray.handle.dispose();

    const reach = boot(makeDeps({ devFlags: { mineReach: true } }));
    expect(reach.rec.rendererArgs[0][4]).toEqual({ aimRay: false, mineReach: true, mineTimer: false, aiContact: false, blockedFire: null, playerColor: '#hex-blue', playerSkin: 'solid', playerAccent: null, quality: QUALITY_PRESETS.high, enemyDeathPulse: false, mineWarn: null, identityMarker: null, shellTrail: null, arrival: null, enemyRole: null, wreck: null });
    reach.handle.dispose();

    const timer = boot(makeDeps({ devFlags: { mineTimer: true } }));
    expect(timer.rec.rendererArgs[0][4]).toEqual({ aimRay: false, mineReach: false, mineTimer: true, aiContact: false, blockedFire: null, playerColor: '#hex-blue', playerSkin: 'solid', playerAccent: null, quality: QUALITY_PRESETS.high, enemyDeathPulse: false, mineWarn: null, identityMarker: null, shellTrail: null, arrival: null, enemyRole: null, wreck: null });
    timer.handle.dispose();

    const contact = boot(makeDeps({ devFlags: { aiContact: true } }));
    expect(contact.rec.rendererArgs[0][4]).toEqual({ aimRay: false, mineReach: false, mineTimer: false, aiContact: true, blockedFire: null, playerColor: '#hex-blue', playerSkin: 'solid', playerAccent: null, quality: QUALITY_PRESETS.high, enemyDeathPulse: false, mineWarn: null, identityMarker: null, shellTrail: null, arrival: null, enemyRole: null, wreck: null });
    contact.handle.dispose();
  });

  it('threads the arrival dev flag through to the renderer (issue #230)', () => {
    // The shipped spawn entrance and death pulse BOTH expand a ring, which is why the issue
    // says they are hard to tell apart at speed. `opposed` converges the entrance and
    // detonates the death; this asserts only that the choice reaches the renderer, because
    // a flag that parses, validates and reports as accepted while changing nothing is the
    // worst failure shape for an experiment -- the owner would conclude the language does
    // not help rather than that it never ran.
    const h = boot(makeDeps({ devFlags: { arrival: 'opposed' } }));
    expect((h.rec.rendererArgs[0][4] as { arrival?: string | null }).arrival).toBe('opposed');
    h.handle.dispose();

    const off = boot();
    expect((off.rec.rendererArgs[0][4] as { arrival?: string | null }).arrival).toBeNull();
    off.handle.dispose();
  });

  it('threads the enemyRole dev flag through to the renderer (issues #357, #773)', () => {
    // EVERY cue, not one: the wiring that hands the renderer a hardcoded 'both' passes a
    // single-arm test, and the six single levers exist precisely to be compared.
    for (const cue of ENEMY_ROLE_CUES) {
      const h = boot(makeDeps({ devFlags: { enemyRole: cue } }));
      const options = h.rec.rendererArgs[0][4] as { enemyRole?: string | null };
      expect(options.enemyRole, cue).toBe(cue);
      h.handle.dispose();
    }
    // ...and absent stays absent: the shipped board draws no cue at all, which is what keeps
    // every tank's geometry exactly where it ships.
    const off = boot();
    expect((off.rec.rendererArgs[0][4] as { enemyRole?: string | null }).enemyRole).toBeNull();
    off.handle.dispose();
  });
  ...
  it("threads the wreck dev flag's chosen arm through to the renderer (issue #232)", () => {
    // Three arms exist to be compared in play, so the renderer must hear WHICH one: a wiring
    // that passed a truthy value would build 'sink' for every arm and the comparison would
    // report that all three feel the same.
    const tilt = boot(makeDeps({ devFlags: { wreck: 'tilt' } }));
    expect((tilt.rec.rendererArgs[0][4] as { wreck?: string | null }).wreck).toBe('tilt');
    tilt.handle.dispose();

    const off = boot();
    expect((off.rec.rendererArgs[0][4] as { wreck?: string | null }).wreck).toBeNull();
    off.handle.dispose();
  });
...
describe('startGameWith: the dev console surface', () => {
  ...
  describe('the benchmark measures a session without changing what it simulates (issue #734)', () => {
    ...
    //
    // UNEVEN steps, on purpose. At a steady 100 ms every frame owes the same whole ticks, and a
    // mutation that shifts the timestamp the driver steps from by 1 ms
    // (`bench-raf-shifts-the-frame-clock`) left the snapshots identical. A 50/17/100 ms cycle,
    // 10.02 ticks long, did not catch it either. 23/41/88 ms is 9.12 ticks per cycle, so the
    // accumulator drifts a whole tick about every 1.3 s of the run, and that shift then fails
    // the comparison below.
    const STEPS_MS = [23, 41, 88];
    const NOW_SEQUENCE = Array.from({ length: 150 }, (_, i) => i).reduce<number[]>(
      (times, i) => [...times, (times.at(-1) ?? 0) + STEPS_MS[i % STEPS_MS.length]],
      [],
    );
