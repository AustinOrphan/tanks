 ...
 * states a direction for them ("retain the current two-mine capacity", "none") and
 * pp1-roles.test.ts pins each against `configFor`, so the arm's "matches" claim fails loudly
 * if the shipped value ever moves, instead of the table quietly ceasing to be the matrix.
 */
