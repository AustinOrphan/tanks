 ...
 * introduction. Each tank definition used to author a `firstMission`, and nothing checked
 * it against the campaign, so it drifted -- Teal said 5 and spawned in level 1. This is
 * DERIVED instead, per campaign, so a campaign edit cannot leave a stale number behind and
 * a second campaign gets its own answer rather than one global value.
 *
