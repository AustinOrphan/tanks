...
describe('VERSUS_CATALOG', () => {
  it('ships 6 entries whose ids equal their arena ids, in arena order', () => {
    ...
    //
    // It reached 8 with vs-tri-01 (#272) and vs-quad-01 (#273), and is back to 6: both are
    // WITHDRAWN pending #424/#425, because human playtesting found players cannot leave
    // their spawns on either board (#423 has the measurement -- each Keystone spawn reaches
    // 2.4% or less of the tank-legal floor, each Quarters spawn about 11%). Their arena
    // definitions remain in `arenas.json`; only the OFFER is withdrawn, so a redesign has
    // something to edit and this catalog stops shipping an unplayable map.
  ...
  it('declares the player counts each entry is CURATED for, not merely the ones it passes at', () => {
    ...
    const CURATED_COUNTS: Record<string, number[]> = {
      ...
      'vs-duel-01': [2],
      // vs-tri-01 (#272) and vs-quad-01 (#273) had rows here declaring [3] and [4]. Both
      // are withdrawn pending #424/#425; see the population pin above. When either returns
      // it needs a row here AND must pass the tank-egress gate in versus-board.test.ts,
      // which is what would have caught them (#423).
    };
    ...
    for (const e of VERSUS_CATALOG) {
      ...
      // Uniform again while vs-tri-01 -- the only entry that ever declared `ffa` alone,
      // three players having no fair team split -- is withdrawn.
