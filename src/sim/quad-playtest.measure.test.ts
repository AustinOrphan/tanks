...
function playMatch(mode: GameMode, seed: number) {
  ...
  // this board 2v2. A four-player board is the first that can be split unfairly, and
  // `teamOf(slot) = slot % 2` over the picker's order is what decides it.
  ...
  it('records observations for every declared mode', () => {
    for (const mode of ['ffa', 'teams'] as GameMode[]) {
      for (const seed of [7, 11, 23]) {
        ...
        //     than repeating the ffa run twice. Without it a `teams` match that silently
        //     stamped nobody, or split 3v1, would log and pass identically to `ffa`. The
        //     ffa branch asserts the complement -- no team stamped at all -- so neither
        //     direction can drift into the other unnoticed.
        if (mode === 'teams') {
          expect(o.teamSizes, `${mode} seed=${seed}: an even 2v2 split`).toEqual([2, 2]);
        } else {
