...
describe('loadArena', () => {
  ...
  // stay byte-identical to today's single-arg call -- across all 6 shipped arenas,
  ...
  it('at playerCount 1 (default and explicit) is byte-identical to the single-arg call, on all 6 shipped arenas', () => {
  ...
  // is open ('.', in-bounds) in all 6 of 6 shipped arenas. arena-01/02/03 share a P
  ...
  // 4)` on all 6 shipped arenas, asserting the CONCRETE resolved cell rather than merely
...
describe('loadArena: mode-aware (n-player arc PR 4 -- FFA + teams)', () => {
  ...
  it('strips every non-player spawn letter in ffa/teams, on all 6 shipped arenas: only player tanks are ever instantiated', () => {
    for (const arena of ARENAS) {
  ...
  it('teams stamps team = slot % 2 on every player tank, N=4, on all 6 shipped arenas', () => {
    for (const arena of ARENAS) {
