 ...
 *
 * WHY THIS FILE EXISTS. #359 gave the campaign AI a committed opponent held for a
 * deterministic window, and versus bots could not have one: they occupy player slots, so
 * `tank.kind === 'player'`, and `stepAi` skipped every such tank by construction. The bot was
 * driven entirely by `player-profile.ts`, which reassessed its opponent every tick. #359's own
 * last acceptance criterion asks for VS-bot evidence of sticky targeting, and until now that
 * evidence could not exist -- a versus recording with `--flag aiContact` drew an empty overlay.
 *
 ...
 * records as an inert value the schema demands and nobody chose (issue #891, point 1).
 *
 ...
 * that band lives on the same inert player profile -- so ranking by raw distance is the
