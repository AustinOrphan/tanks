...
the shared pipeline at commit `830e3f53e2fc0047f464127951b664dd27ba4a6d`, from a build stamped with that same commit
(`VITE_BUILD_SHA=830e3f53e2fc0047f464127951b664dd27ba4a6d npm run build`). Each capture's own `capture.json` is beside it.

| File | Recipe | Frames | Round |
| --- | --- | --- | --- |
| `pp1roles-off.mp4` | `flow.campaign-round.pp1roles-off` | 150 at 30 fps | 5.01 s, 303 simulated ticks |
| `pp1roles-on.mp4` | `flow.campaign-round.pp1roles-on` | 165 at 30 fps | 5.53 s, 332 simulated ticks |
...
- `flow-simulation-rate`: 60.24 and 59.97 simulated ticks per wall-clock second.
...
- `flow-build-identity`: the page named `830e3f53e2fc0047f464127951b664dd27ba4a6d`, the commit captured.
- `flow-delivered-rate`: 98.9 and 98.6 compositor frames per second, against the recipe's
