...
- `docs/README.md` is the routing index for the document corpus. It is GENERATED by
  `tools/docs/index.mjs`; run `npm run docs:index` after adding, retitling, restatusing, or
  deleting a plan, specification, backlog topic, or research entry. Backlog topics are
  counted there and enumerated only in `docs/superpowers/backlog.md`; never list them twice.
...
- Generate `docs/README.md`, `docs/dev-flags.md`, and the tank list through their repository
  generators. Never repair a drift test by editing generated output or weakening the guard.
