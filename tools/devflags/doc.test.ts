...
// (docs/agent/architecture.md: "Quote a measurement in `notes` and you owe it a recomputing
// test") applied to a whole file instead of a number in prose: regenerate in memory, compare
// against what's committed, and fail if they differ. The fix for a red run here is `npm run
// devflags:doc`, never editing this test or the doc by hand -- editing either to make
// this pass is exactly the "repair the red build by changing what the test expects" habit
// docs/agent/known-holes.md warns ends guards.
