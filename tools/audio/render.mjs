...
/**
 * The whole CLI, in a function so that IMPORTING this module does not run it (issue #881).
 *
 * `tools/mutate` measures an entry through Vitest's own dependency graph, so an entry on this
 * file is refused outright unless some test imports it -- and until this wrapper existed, an
 * import spawned vite, launched Chromium and rendered a .wav.
 *
 * The body below is UNCHANGED from when it ran at the top level: `git diff -w` on the commit
 * that added this shows only these wrapper lines and the guard at the foot. What proves it is
 * not the diff though -- `--sfx explosion` and `--sfx all` render byte-identically across the
 * change, and those two targets are bit-reproducible in the first place.
 */
async function main() {
...
// `tools/screens/record.mjs` has carried since issue #815.
