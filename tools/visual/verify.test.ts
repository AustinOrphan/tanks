...
// outright. This import is what puts `verify.mjs` in that graph.
//
// And behaviourally: before #881 this module called `main()` at the top level, so importing it
// HERE would launch a browser and run the required visual gate itself. That it does not is the guard
// working; if the guard regressed, this suite would hang rather than fail cleanly.
import './verify.mjs';

describe('verify.mjs: the seams no vitest run can execute', () => {
  ...
  it('installs the override on BOTH of its page paths, before either navigates', () => {
    // Issue #877. Exactly four and eight spaces: `\s*` would accept the call nested inside an
    ...
    expect(src, 'the override is gone, conditional, or no longer on its own line').toMatch(
      /^ {4}await context\.addInitScript\(audioContextOverrideSource\(\)\);$/m,
    );
    expect(src, 'the override is gone, conditional, or no longer on its own line').toMatch(
      /^ {8}await page\.addInitScript\(audioContextOverrideSource\(\)\);$/m,
    );
    const at = src.search(/^ {4}await context\.addInitScript\(audioContextOverrideSource\(\)\);$/m);
